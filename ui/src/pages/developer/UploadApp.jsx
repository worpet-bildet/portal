...
export function UploadApplication(props) {
  ...
  const handleUpdate = devApps => {
    const currentApp = getApplications(devApps).find(
      app => app.key["app-name"] == application
    );
...
function Form({ application, name }) {
  ...
  const redirectToMain = () => {
    navigate("/apps/portal/dev/");
    Notify.success(
      `To upload app icon from docket data use the following command from the ship which hosts the app: :dst-server|send-data [~sampel-dozdev %app-name]`
    );
    Notify.success(
      `If necessary, send a signature from the ship which hosts the app with: :dst-server|sign [~sampel-dozdev %app-name]`
    );
  };
...
function AppPageInformation({ setDisableForm }) {
  ...
  return (
    <div>
      <div className="mb-6">
        <div className="flex flex-row gap-14">
          <div className="w-full">
            <div>
              <small>*must be the same as your desk name, e.g. app-name</small>
            </div>
      ...
      <div>
        <small>
          If this isn't your current ship, add a signature after saving the app, or fill
          out this form from the distributor ship.
        </small>
      </div>
      <Input
        label="Distributor desk "
        placeholder="~sampel/{your app name}"
        {...register("dev-input.dst-desk")}
      />
    </div>
  );
}
  ...
  return (
    <div className="mb-3">
      ...
      <h1 className="text-1xl mb-4">
        Signatures are only required when you fill out this form from a ship other than
        the one that hosts the app desk. They ensure that the ship filling out the form
        has the correct permission to distribute the desk.
      </h1>
      <a
        ...
        <div className="mb-6 mt-10">
          ...
          <div className="grid grid-cols-2 gap-x-8 mt-3">
            <div className="mb-3 basis-1/2">
              <Input
                label="Title"
                placeholder={application.docket.title}
                disabled={true}
              />
            </div>
            <div className="mb-3 basis-1/2">
            ...
            <div className="mb-3 basis-1/2">
              <Input
                label="Color"
                placeholder={application.docket.color}
                disabled={true}
              />
            </div>
            <div className="mb-3 basis-1/2">
              <Input
                label="Href"
                placeholder={application.docket.href.site}
                disabled={true}
              />
            </div>
            <div className="mb-3 basis-1/2">
              <Input
                label="Image"
                placeholder={application.docket.image}
                disabled={true}
              />
            </div>
            <div className="mb-3 basis-1/2">
              <Input
                label="Version"
                placeholder={application.docket.version}
                disabled={true}
              />
            </div>
            <div className="mb-3 basis-1/2">
              <Input
                label="Website"
                placeholder={application.docket.website}
                disabled={true}
              />
            </div>
            <div className="mb-3 basis-1/2">
              <Input
                label="License"
                placeholder={application.docket.license}
                disabled={true}
              />
            </div>
          </div>
...
const ArrayInput = React.forwardRef(
  ({ label, name, placeholder, setDisableForm, ...rest }, ref) => {
    ...
    const isStartingWithNumber = keywords => {
      const startsWithNumber = keywords
        .map(keyword => !!keyword.match(/^[0-9]/))
        .filter(startsWithNumber => startsWithNumber);
      ...
      <>
        ...
        <ul className="flex justify-start mt-2 gap-x-1 flex-wrap">
          {getValues("dev-input.keywords") && !startsWithNumber ? (
            getValues("dev-input.keywords").map((keyword, index) => (
              <Tag key={keyword + index} name={keyword} />
            ))
          ) : (
            <Alert
              color="red"
              message="Keywords cannot start with a number"
...
const ImageInput = React.forwardRef(
  ({ name, label, placeholder, onClick, index, ...rest }, ref) => {
    return (
