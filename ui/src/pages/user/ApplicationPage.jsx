...
export function ApplicationPage(props) {
  ...
  const handleUpdate = curators => {
    ...
    const currentApp = getApplications(curators).find(
            ...
            <div className="flex flex-col gap-2">
              ...
              {appInfo ? (
                <>
                  ...
                  <p className="text-lg text-justify mt-5">{appInfo.description}</p>
                  <Tabs
                  ...
                  {selectedButton === "Reviews" && (
                    <Reviews
                      ...
                      appKey={appInfo.key}
                      hash={appInfo["desk-hash"]}
                    />
                  )}
                  {selectedButton === "Comments" && (
                    <Comments comments={appInfo.comments} appKey={appInfo.key} />
                  )}
                </>
              ) : null}
            </div>
...
function Review({ text, user, appKey, isSafe }) {
  ...
  const executeAction = review => {
    api.poke({
      ...
      json: { "del-rev": review },
      onSuccess: () =>
        Notify.success("Your review has been deleted, please refresh the page"),
...
function CommentForm({ appKey }) {
  ...
  const submitNew = comment => {
    api.poke({
      ...
      onSuccess: () =>
        Notify.success(
          "Your comment has been processed succesfully, please refresh the page"
        ),
      onError: err => Notify.error(err),
    });
  };
