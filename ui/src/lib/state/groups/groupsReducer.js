function reduceCordon(draft, diff) {
  const d = diff.cordon;
...
function reduceFleet(draft, diff) {
  ...
  if ("add" in d) {
    ...
    if ("shut" in draft.cordon) {
      const addsRemoved = draft.cordon.shut.pending.filter(s => ships.includes(s));
...
export default function groupsReducer(flag, data) {
  return draft => {
    const { diff } = data;
    ...
    if ("channel" in diff) {
      ...
      if ("add" in d) {
        ...
        group.zones[oldZone].idx = group.zones[oldZone].idx.filter(n => n !== nest);
