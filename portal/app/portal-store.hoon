...
++  handle-poke  ::  all arms here should output [cards items]
    ...
    ++  prepend-to-feed
      ...
      ?.  =(time.feed-key.act 'global')
        ...
        (~(poke pass:io /msg) [~bus %portal-store] portal-message+!>(msg))
      :-  (welp cards cards-1)
      state
    ::
    ++  append  ::  deduplicates
...
++  init-sequence
  ^+  [*(list card) state]
  =/  feed-path  [%item %feed '~bus' '' 'global' ~]
  =^  cards  item-sub  (surf:da-item ~bus %portal-store feed-path)
  =.  cards  (welp cards (track-gr:cards-methods ~bus))
  =^  cards-1  state
    %-  create:handle-poke
    :*  %create  ~  ~  `'~2000.1.1'  `%def
